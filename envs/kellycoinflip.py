# Adapted from pzhokhov's github repository. 
# Changes include:
#   - the action space is now continuous (not discrete)
#   - the observation gym space type is a Box, not a Tuple
#   - bets are made as a fraction of the total wealth
#   - Add an option to keep rewards sparse, or for the reward to be the change in wealth for each period
...
class KellyCoinflipEnv(gym.Env):
    ...
    def __init__(self, initial_wealth=25.0, edge=0.6, max_wealth=250.0, max_rounds=300, sparse_rewards=False ):

        # Action is the bet size
        self.action_space = spaces.Box( low=np.array( [0.], dtype=np.float32 ), 
                                        high=np.array( [1.], dtype=np.float32 ) )
        # Observation space is:
        # 1) current wealth; 2) rounds elapsed
        self.observation_space = spaces.Box( low=np.array( [0., 0.], dtype=np.float32 ), 
                                             high=np.array( [max_wealth, max_rounds ], dtype=np.float32 ) )
    ...
    def step(self, action):
        bet_in_dollars = action * self.wealth  # action = desired bet
        self.rounds -= 1

        coinflip = flip(self.edge, self.np_random)
        orig_wealth = self.wealth
        self.wealth = min( self.max_wealth, self.wealth + coinflip * bet_in_dollars )

        done = np.isclose( self.wealth, 0. ) or self.wealth >= self.max_wealth or not self.rounds
        if self.sparse_rewards:
            reward = self.wealth if done else 0.0
        else:
            reward = self.wealth - orig_wealth

        return self._get_obs(), reward, done, {}

    def _get_obs(self):
        return np.array( [ self.wealth, self.rounds ], dtype=np.float32 ) 
...
class KellyCoinflipGeneralizedEnv(gym.Env):
    ...
    def __init__(self, initial_wealth=25.0, edge_prior_alpha=7, edge_prior_beta=3,
                 ...
                 max_rounds_sd=25.0, reseed=True, sparse_rewards=False):
        ...
        self.max_ever_wealth = self.initial_wealth
        ...
        # Action is just the bet size
        self.action_space = spaces.Box( low=np.array( [0.], dtype=np.float32 ), 
                                        high=np.array( [1.], dtype=np.float32 ) )
                                       
        # Observation space is:
        # 1) current wealth; 2) rounds elapsed; 3) wins; 4) losses; 5) maximum observed wealth
        self.observation_space = spaces.Box( 
                         low=np.array( [0., 0., 0., 0., 0. ], dtype=np.float32 ), 
                         high=np.array( [max_wealth, max_rounds, max_rounds, max_rounds, max_wealth ], dtype=np.float32 ) )
        self.reward_range = (0, max_wealth)
        self.edge = edge
        self.wealth = self.initial_wealth
        self.max_rounds = max_rounds
    ...
    def step(self, action):
        bet_in_dollars = action * self.wealth
    ...
    def _get_obs(self):
        return np.array( [ self.wealth, self.rounds_elapsed, self.wins, self.losses, self.max_ever_wealth ], 
                                                                                        dtype=np.float32 )
